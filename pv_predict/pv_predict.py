...
for num_rows in range(1000, 17600, 100):
    ...
    trimmed_data=pd.read_csv('input_data2022.csv').head(num_rows)
    ...
    def get_feature_gain():
        ...
        df = pd.read_csv('input_data2022.csv').head(num_rows)
        train = df
    ...
    for i in range(p.N_VERIFICATION):
        for day in range(1,p.DAYS):
         ...
         # output prediction intervals using LUBE
            ...
            print(f"最終的な損失関数の値: {final_loss_value:.4f}")
    ...
    # データフレームの初期化
    df = pd.DataFrame(columns=["cpu_info", "memory_info", "cpu_clock_frequency", "DAYS", "number_of_data", "elapsed_time"])
    ...
    execution_df = pd.DataFrame({
        ...
        "DAYS": [p.DAYS],
        "number_of_data": len(trimmed_data),
    ...
    print("CPU情報、メモリ情報、CPUクロック周波数、DAYS、データの数、実行時間がCSVファイルに記録されました。")  # メッセージに「データの数」を追加