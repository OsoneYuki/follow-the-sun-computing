...
import schedule_parameters as p


warnings.simplefilter('ignore')

class ESS_Model(gym.Env):
    def __init__(self, mode, pdf_day, train_days, test_day, PV_parameter, action_space, num_rows):
        #パラメータの定義
        self.episode = 0
...
cpu_clock_frequency = p.CPU_CLOCK_FREQUENCY


    #パラメータ設定
    #30分1コマで、何時間先まで考慮するか
for action_space in p.ACTION_SPACE:  #アクションの数(現状は48の約数のみ)
    num_episodes = int(48/action_space) # 1Dayのコマ数(固定)

    # 学習回数
    for episode in p.EPISODE:

        

        # test 1Day　Reward最大
        pdf_day = 0 #確率密度関数作成用のDay数 75 80
        for train_days in p.TRAIN_DAYS: # 学習Day数 70 ~ 73
            for test_day in p.TEST_DAY: # テストDay数 + 2 (最大89)
                ...
                # Training環境設定と実行
                env = ESS_Model(mode, pdf_day, train_days, test_day, PV_parameter, action_space, num_rows)
                ...
                # データフレームの初期化
                df = pd.DataFrame(columns=["cpu_info", "memory_info", "cpu_clock_frequency",  "number_of_data", "episode", "action_space", "train_days", "test_day", "execution_time"])
